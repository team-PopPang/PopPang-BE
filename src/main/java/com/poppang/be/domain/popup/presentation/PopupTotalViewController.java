...
public class PopupTotalViewController {
    ...
    @Operation(
            summary = "팝업 상세 진입 시 조회수 증가",
            description = "특정 팝업의 상세 화면에 진입할 때 조회수를 1 증가시킵니다."
    ...
    @Operation(
            summary = "팝업 총 조회수 조회",
            description = "특정 팝업의 누적 조회수를 반환합니다."
    )
    @GetMapping("/{popupUuid}/total-view-count")
    public ResponseEntity<PopupTotalViewCountResponseDto> getTotalViewCount(@PathVariable String popupUuid) {
        PopupTotalViewCountResponseDto popupTotalViewCountResponseDto = popupTotalViewCountService.getTotalViewCount(popupUuid);
